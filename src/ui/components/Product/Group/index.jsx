    ...
    if (item && onAddOrder) {
      return (
        <Popover
          placement="bottom"
          isOpen={true}
          target={`product-${item.item_uuid}`}
          toggle={() => this.setState({ selectedItem: null })}
        >
          <PopoverHeader>{item.title}</PopoverHeader>
          <PopoverBody>
            <ProductOptions
              inline={false}
              onAddOrderItem={onAddOrder}
              canAddOrder={true}
              item={item}
            />
          </PopoverBody>
          ...
          displayProducts.map((item, index) => (
            ...
            >
              <ProductItem
                ...
                title={
                  <span id={`product-${item.item_uuid}`}>{item.name}</span>
                }
                image={this.getProductImage(item.gallery)}
                item_slug={item.slug || item.item_uuid}
                onIncrease={
                  onAddOrder
                    ? () => this.processAddOrder(item, onAddOrder)
                    : null
                }
              />
            </div>
